...
| A restic snapshot backed up `/etc` instead of `/mnt/dc01-share` under the `dc01-share` tag — a leftover manual test predating `backup-dc01.sh` | Found while reviewing `restic snapshots` output for the portfolio screenshots. Retention alone would not have removed it: restic's `forget` policy evaluates snapshots by date and tag, not by path, and had already kept this one as the oldest snapshot satisfying the weekly/monthly rules. Checked with `restic ls` for `restic-pass` and `dc01-creds` before doing anything else — neither was present — then removed explicitly by snapshot ID with `restic forget <id> --prune`, not left for retention to decide |
